...
-- How many unique products lines does a data have?
SELECT *FROM sales;
...
-- What product city has the largest revenue?
...
-- Which cutomer pays the most in VAT?
...
SELECT DISTINCT customer_type, COUNT(*) AS cnt
FROM sales GROUP BY customer_type
ORDER BY cnt DESC;
...
-- Which time of the day do custSELECT time_of_day, AVG(rating) AS avg_rating
SELECT time_of_day, AVG(rating) AS avg_rating
FROM sales WHERE branch = "B"
GROUP BY time_of_day
ORDER BY avg_rating DESC;

-- Which day of the week have the best average rating?
